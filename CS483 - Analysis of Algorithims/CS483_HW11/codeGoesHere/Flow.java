//You can (and probably should) edit this file...
//Just because this demo prints out a bunch of stuff
//doesn't mean you need that happening all the time
//while you're working!

//Hint for those who have never done this before...
//adding a boolean to the class called something like
//"debugMode" and then replace-all "if(printOn) System.out.print" with
//"if(debugMode) if(printOn) System.out.print" tends to work rather well.
//You might only need to do a little cleanup after that.
...
class Flow {
	...
	public Flow(int[][] graph, int source, int sink, boolean printOn) {
		...
		if(printOn) System.out.println("Flow from " + source + " to " + sink + " in graph: ");
		if(printOn) printGraph(graph);
	...
	public void computeMaxFlowFordFulkerson() {
		...
		while(pathStart != null) {
			...
			while(curr.next != null) {
				if(printOn) System.out.print(curr.value.startNodeId + "->");
				curr = curr.next;
			}
			if(printOn) System.out.print(curr.value.startNodeId + "->");
			if(printOn) System.out.println(curr.value.endNodeId);
			
			//get the minimum cost of the path
			int minCost = getMinCost(pathStart);
			if(printOn) System.out.println("Min Cost: " + minCost);
	...
	public Node getPath() {
		...
		for(int i = 0; i < queue.length; i++) {
			if(printOn) System.out.print(queue[i] + " ");
		}
		if(printOn) System.out.println();
		
		if(printOn) System.out.print("Parents: ");
		for(int i = 0; i < parents.length; i++) {
			if(printOn) System.out.print(parents[i] + " ");
		}
		if(printOn) System.out.println();
