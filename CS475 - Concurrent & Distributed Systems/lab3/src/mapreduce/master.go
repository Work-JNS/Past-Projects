...
type Master struct {
	...
	arg     string   // input arguments (if any) for map function
...
func Sequential(jobName string, arg string, files []string, nreduce int,
	mapF func(string, string, string) []KeyValue,
	reduceF func(string, []string) string,
) (mr *Master) {
	mr = newMaster("master")
	go mr.run(jobName, arg, files, nreduce, func(phase jobPhase) {
		switch phase {
		case mapPhase:
			for i, f := range mr.files {
				doMap(mr.jobName, i, arg, f, mr.nReduce, mapF)
			}
		case reducePhase:
...
// master over RPC.
func Distributed(jobName string, arg string, files []string, nreduce int, master string) (mr *Master) {
	mr = newMaster(master)
	mr.startRPCServer()
	go mr.run(jobName, arg, files, nreduce, mr.schedule, func() {
...
func (mr *Master) run(jobName string, arg string, files []string, nreduce int,
...
func (mr *Master) killWorkers() []int {
	...
	for _, w := range mr.workers {
		...
		if ok == false {
			debug("Master: RPC %s shutdown error\n", w)
		} else {
