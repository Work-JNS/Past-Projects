...
void print_value(const char *msg, microfp_s value) {
  printf("\r%s%s: %s", GREEN, msg, CYAN);
  print_binary_digits_precision(value, MICROFP_TOTAL_BITS);
  printf(" (0x%03x)%s%c", value & ((1 << MICROFP_TOTAL_BITS)-1), RST, IS_INTERACTIVE?'\n':'\0');
}
...
static void print_binary_digits_precision(unsigned int val, int precision) {
  if(precision == 0) {
    ...
    printf("%d", val&0x1);
    if(precision == MICROFP_SIGN_BITS) printf(" ");
    else if(precision == MICROFP_SIGN_BITS + MICROFP_EXP_BITS) printf(" ");
  }
} 
